...
set_os_params()
{
	...
	#yum update -y
	dnf install libevent gcc kernel-devel m4 flex wget zip bzip2 krb5-devel xfsdump expect cloud-init tk bc psmisc cloud-init epel-release -y
...
get_edb_binaries()
{
	...
	curl -1sSLf "https://downloads.enterprisedb.com/${EDB_SUBSCRIPTION_TOKEN}/analytics/setup.rpm.sh" | sudo -E bash
	dnf install -y warehouse-pg-7 warehouse-pg-clients whpg-backup edb-whpg7-pxf edb-whpg7-pgaa edb-whpg7-madlib java
